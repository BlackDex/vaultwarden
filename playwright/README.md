...
# Locally
npx playwright codegen "https://127.0.0.1:8003"
...
- `Vaultwarden` on https://0.0.0.0:8003 with the default user `test@yopmail.com/test`.
- `Keycloak` on https://0.0.0.0:8443/admin/master/console/ with the default user `admin/admin`
...
- `keycloak` [configuration](https://www.keycloak.org/server/all-config) includes `KC_BOOTSTRAP_ADMIN_USERNAME` / `KC_BOOTSTRAP_ADMIN_PASSWORD` and any variable prefixed `KC_` ([more information](https://www.keycloak.org/server/configuration#_example_configuring_the_db_url_host_parameter)).
